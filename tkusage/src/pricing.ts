...
export function getPricingRate(source: UsageSource, model: string | null): PricingRate | null {
    ...
    if (source === 'claude') {
        return getClaudePricingRate(model);
    }

    if (source === 'codex') {
        return getCodexPricingRate(model);
    }
...
export function getPricingNote(): string {
    return 'Estimated USD using embedded Claude and Codex token pricing plus native OpenClaw session-log costs when available. Claude totals include sidechains/subagents by default unless --main-thread-only is set. Not a vendor billing statement.';
}
