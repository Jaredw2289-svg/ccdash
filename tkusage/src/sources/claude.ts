...
interface NormalizedClaudeUsageOptions {
...
function parseSessionRelativeDir(filePath: string, transcriptRoot: string): string {
    const relativeFilePath = path.relative(transcriptRoot, filePath);
    const pathParts = relativeFilePath.split(path.sep).filter(Boolean);

    if (pathParts.length >= 3 && pathParts[pathParts.length - 2] === 'subagents') {
        const projectParts = pathParts.slice(0, -3);
        return projectParts.length === 0 ? '.' : projectParts.join(path.sep);
    }
...
function isSubagentTranscriptFile(filePath: string): boolean {
...
function shouldSkipClaudeRecord(parsed: unknown, filePath: string, options: NormalizedClaudeUsageOptions): boolean {
...
function parseUsageFile(
    ...
    options: NormalizedClaudeUsageOptions
): UsageRecord[] {
...
function dedupeClaudeRecords(records: UsageRecord[]): UsageRecord[] {
    const recordsByRequestId = new Map<string, UsageRecord>();
    const recordsWithoutRequestId: UsageRecord[] = [];
    ...
    return [...recordsByRequestId.values(), ...recordsWithoutRequestId]
        .sort((left, right) => left.timestamp.localeCompare(right.timestamp));
}

function normalizeClaudeUsageOptions(options?: ClaudeUsageOptions): NormalizedClaudeUsageOptions {
    return {
