...
**A customizable session dashboard for Claude Code CLI**
*Display model info, git status, token usage, session goals, and more in your terminal*

[![npm version](https://img.shields.io/npm/v/ccdash.svg)](https://www.npmjs.com/package/ccdash)
[![npm downloads](https://img.shields.io/npm/dm/ccdash.svg)](https://www.npmjs.com/package/ccdash)
[![License: MIT](https://img.shields.io/badge/License-MIT-blue.svg)](https://github.com/Jaredw2289-svg/ccdash/blob/main/LICENSE)
[![Node.js Version](https://img.shields.io/node/v/ccdash.svg)](https://nodejs.org)
[![install size](https://packagephobia.com/badge?p=ccdash)](https://packagephobia.com/result?p=ccdash)
[![Maintenance](https://img.shields.io/badge/Maintained%3F-yes-green.svg)](https://github.com/Jaredw2289-svg/ccdash/graphs/commit-activity)

[![Mentioned in Awesome Claude Code](https://awesome.re/mentioned-badge.svg)](https://github.com/hesreallyhim/awesome-claude-code)
[![ClaudeLog - A comprehensive knowledge base for Claude](https://claudelog.com/img/claude_log_badge.svg)](https://claudelog.com/)


![Demo](https://raw.githubusercontent.com/Jaredw2289-svg/ccdash/main/screenshots/demo.gif)
...
- **⚙️ Custom config file flag (v2.1.8)** - Added `--config <path>` support so ccdash can load/save settings from a custom file location.
...
![Emoji Support](https://raw.githubusercontent.com/Jaredw2289-svg/ccdash/main/screenshots/emojiSupport.png)
...
![Powerline Auto-Alignment](https://raw.githubusercontent.com/Jaredw2289-svg/ccdash/main/screenshots/autoAlign.png)
...
![Current Working Directory and Session Cost](https://raw.githubusercontent.com/Jaredw2289-svg/ccdash/main/screenshots/cwdAndSessionCost.png)

- **📁 Current Working Directory** - Display the current working directory with configurable segment display
  - Set the number of path segments to show (e.g., show only last 2 segments: `.../Personal/ccdash`)
...
![Block Timer](https://raw.githubusercontent.com/Jaredw2289-svg/ccdash/main/screenshots/blockTimerSmall.png)
...
> 💡 **Tip:** Your settings are automatically saved to `~/.config/ccdash/settings.json`
...
When you select Install from the TUI, ccdash automatically configures everything:

1. **Status line command** → `~/.claude/settings.json` — tells Claude Code to use ccdash for rendering
2. **Hooks** → `~/.claude/settings.json` — registers a `UserPromptSubmit` hook that caches the session ID
3. **Session dashboard instructions** → `~/.claude/CLAUDE.md` — tells Claude to write goal/now summaries after each response

This means **zero manual configuration** — just run `npx ccdash`, install, and start using Claude Code.

### Session Dashboard (Goal / Now)

ccdash can show a live session goal and current status on your status bar:

- **Goal** (L3) — a stable session objective, written on the first turn and updated when the goal changes
- **Now** (L4) — the latest conclusion or current action, updated after every response

These are written by the model itself via a bash command specified in `~/.claude/CLAUDE.md` (auto-installed). The session ID is resolved automatically from a hook cache — no environment variables needed.

If no model-written summary exists, ccdash falls back to extracting summaries from the conversation transcript.

### Claude Code settings.json format

When you install from the TUI, ccdash writes a `statusLine` command object to your Claude Code settings:

```json
{
  "statusLine": {
    "type": "command",
    "command": "npx -y ccdash@latest",
    "padding": 0
  }
}
```
...
- `bunx -y ccdash@latest`
- `ccdash` (for self-managed/global installs)

---

## 🪟 Windows Support

ccdash works seamlessly on Windows with full feature compatibility across PowerShell (5.1+ and 7+), Command Prompt, and Windows Subsystem for Linux (WSL).
...
# Or with Yarn
yarn dlx ccdash@latest

# Or with pnpm
pnpm dlx ccdash@latest
```
...
#### Path Handling
ccdash automatically handles Windows-specific paths:
...
Configure ccdash in your Claude Code settings:
...
> 💡 **Custom Config Directory:** If you use a non-standard Claude Code configuration directory, set the `CLAUDE_CONFIG_DIR` environment variable before running ccdash. The tool will automatically detect and use your custom location.

### Performance on Windows

ccdash includes Windows-specific runtime behavior:
...
Once configured, ccdash automatically formats your Claude Code status line. The status line appears at the bottom of your terminal during Claude Code sessions.
...
- **Status Summary** - Shows session goal or current focus (sources: `goal`, `last-conclusion`, `first-human`, `current-focus`, `session-overview`)
...
![Global Options](https://raw.githubusercontent.com/Jaredw2289-svg/ccdash/main/screenshots/global.png)
...
![Block Timer](https://raw.githubusercontent.com/Jaredw2289-svg/ccdash/main/screenshots/blockTimer.png)
...
> 💡 **Tip:** Custom commands can be other Claude Code compatible status line formatters! They receive the same JSON via stdin that ccdash receives from Claude Code, allowing you to chain or combine multiple status line tools.
...
![ccusage integration](https://raw.githubusercontent.com/Jaredw2289-svg/ccdash/main/screenshots/ccusage.png)
...
This repository also ships an experimental `cdxusage` CLI for local Codex Desktop / Codex CLI session logs stored in `~/.codex/sessions`.

Examples:

```bash
# Local development
bun run cdxusage -- daily
bun run cdxusage -- monthly 2026-03 --breakdown
bun run cdxusage -- session --compact

# Override CODEX_HOME when needed
bun run cdxusage -- daily --codex-home /path/to/.codex
```

`cdxusage` reports:
- daily or monthly token usage
- per-session drill-down
- per-model breakdowns
- API-equivalent USD estimates using embedded GPT-5 Codex pricing
- latest observed Codex plan utilization percentages from local session logs
...
- Git
- Node.js 14+ (optional, for running the built `dist/ccdash.js` binary or npm publishing)

### Setup

```bash
# Clone the repository
git clone https://github.com/Jaredw2289-svg/ccdash.git
cd ccdash
...
- `~/.config/ccdash/settings.json` - ccdash UI/render settings
- `~/.claude/settings.json` - Claude Code settings (`statusLine` command, hooks)
- `~/.claude/CLAUDE.md` - Session dashboard instructions (auto-managed by ccdash)
- `~/.claude/status/{session_id}.json` - Per-session goal/now state
- `~/.cache/ccdash/session-by-term/{term_id}` - Cached session ID per terminal
- `~/.cache/ccdash/block-cache-*.json` - block timer cache (keyed by Claude config directory hash)

If you use a custom Claude config location, set `CLAUDE_CONFIG_DIR` and ccdash will read/write that path instead of `~/.claude`.

### Build Notes

- Build target is Node.js 14+ (`dist/ccdash.js`)
...
[MIT](LICENSE) — originally by Matthew Breedlove ([@sirmalloc](https://github.com/sirmalloc)), forked and maintained by JaredW.
...
- GitHub: [@Jaredw2289-svg](https://github.com/Jaredw2289-svg)
...
<a href="https://www.star-history.com/#Jaredw2289-svg/ccdash&Timeline">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=Jaredw2289-svg/ccdash&type=Timeline&theme=dark" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=Jaredw2289-svg/ccdash&type=Timeline" />
   <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=Jaredw2289-svg/ccdash&type=Timeline" />
 </picture>
</a>
...
[![GitHub stars](https://img.shields.io/github/stars/Jaredw2289-svg/ccdash?style=social)](https://github.com/Jaredw2289-svg/ccdash/stargazers)
[![GitHub forks](https://img.shields.io/github/forks/Jaredw2289-svg/ccdash?style=social)](https://github.com/Jaredw2289-svg/ccdash/network/members)
[![GitHub watchers](https://img.shields.io/github/watchers/Jaredw2289-svg/ccdash?style=social)](https://github.com/Jaredw2289-svg/ccdash/watchers)
...
[![Issues](https://img.shields.io/github/issues/Jaredw2289-svg/ccdash)](https://github.com/Jaredw2289-svg/ccdash/issues)
[![Pull Requests](https://img.shields.io/github/issues-pr/Jaredw2289-svg/ccdash)](https://github.com/Jaredw2289-svg/ccdash/pulls)
[![Contributors](https://img.shields.io/github/contributors/Jaredw2289-svg/ccdash)](https://github.com/Jaredw2289-svg/ccdash/graphs/contributors)

### 💬 Connect

[Report Bug](https://github.com/Jaredw2289-svg/ccdash/issues) · [Request Feature](https://github.com/Jaredw2289-svg/ccdash/issues) · [Discussions](https://github.com/Jaredw2289-svg/ccdash/discussions)
