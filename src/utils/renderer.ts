...
interface RenderToken {
    content: string;
    type: 'content' | 'separator' | 'flex-separator';
}
...
export function renderStatusLine(
    ...
    elements.forEach((elem, index) => {
        ...
        if (elem.type === 'separator' || elem.type === 'flex-separator') {
            finalTokens.push({ content: elem.content, type: elem.type === 'flex-separator' ? 'flex-separator' : 'separator' });
        } else {
            // Check if padding should be omitted due to no-padding merge
            ...
            if (padding && (elem.widget?.backgroundColor || hasColorOverride)) {
                ...
                finalTokens.push({ content: paddedContent, type: 'content' });
            } else if (padding) {
                ...
                finalTokens.push({ content: leadingPadding + elem.content + trailingPadding, type: 'content' });
            } else {
                // No padding
                finalTokens.push({ content: elem.content, type: 'content' });
            }
        }
    });
    ...
    if (overflowBehavior === 'wrap' && effectiveMaxWidth && effectiveMaxWidth > 0) {
        const collapseFlexSep = (token: RenderToken): RenderToken => token.type === 'flex-separator'
            ...
            : token;
        const wrapTokens = hasFlexSeparator
        ...
        const wrappedLines = wrappedGroups.map((group) => {
            let wrappedLine = joinTokens(group);
