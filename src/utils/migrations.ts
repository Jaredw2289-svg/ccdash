...
export const migrations: Migration[] = [
    ...
    },
    {
        ...
        migrate: (data) => {
            ...
            migrated.version = 7;
            migrated.updatemessage = {
                message: 'dashcc updated - dashboard layout refreshed with responsive wrapping',
                remaining: 12
            };
    ...
    },
    {
        fromVersion: 7,
        toVersion: 8,
        description: 'Migrate from v7 to v8 - default dashboard uses stable responsive width tiers',
        migrate: (data) => {
            const migrated: Record<string, unknown> = { ...data };

            if (Array.isArray(migrated.lines)) {
                const lines = migrated.lines as unknown[][];
                if (matchesPreset(lines, DASHBOARD_PRESET) && migrated.flexMode === 'full-minus-40') {
                    migrated.flexMode = 'responsive-stable';
                }
            }

            migrated.version = 8;
            migrated.updatemessage = {
                message: 'dashcc updated - stable terminal width tiers now drive the default dashboard',
                remaining: 12
            };
    ...
    },
    {
        fromVersion: 8,
        toVersion: 9,
        description: 'Migrate from v8 to v9 - add line priority for responsive line dropping',
        migrate: (data) => {
            const migrated: Record<string, unknown> = { ...data };

            migrated.linePriority = [1, 2, 1, 1];
            migrated.version = 9;
            migrated.updatemessage = {
                message: 'dashcc updated - narrow terminals now hide low-priority lines',
                remaining: 12
            };
