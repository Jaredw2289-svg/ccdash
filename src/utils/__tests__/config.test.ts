...
describe('config utilities', () => {
    ...
    it('writes defaults when settings file does not exist', async () => {
        ...
        expect(onDisk.defaultSeparator).toBe('·');
        expect(onDisk.fallbackTerminalWidth).toBe(80);
        expect(onDisk.overflowBehavior).toBe('wrap');
    ...
    it('migrates older versioned settings and persists migrated result', async () => {
        ...
        expect(settings.version).toBe(CURRENT_VERSION);
        expect(settings.fallbackTerminalWidth).toBe(80);
        const migrated = JSON.parse(fs.readFileSync(settingsPath, 'utf-8')) as {
            version?: number;
            updatemessage?: { message?: string };
        };
        expect(migrated.version).toBe(CURRENT_VERSION);
        expect(migrated.updatemessage?.message).toContain('narrow terminals now hide low-priority lines');
