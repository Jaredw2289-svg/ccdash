...
describe('renderer ANSI/OSC handling', () => {
    ...
    it('wraps styled text without breaking OSC 8 hyperlinks', () => {
        const line = `${OSC8_OPEN}alpha beta gamma delta${OSC8_CLOSE}`;
        const wrapped = wrapStyledText(line, 8);

        expect(wrapped.length).toBeGreaterThan(1);
        expect(wrapped.every(part => getVisibleWidth(part) <= 8)).toBe(true);
        expect(wrapped[0]).toContain(OSC8_CLOSE);
    });

    it('wraps mixed-width text by visible columns', () => {
        const wrapped = wrapStyledText('发布成功后 update README and npm tags immediately', 12);

        expect(wrapped.length).toBeGreaterThan(1);
        expect(wrapped.every(part => getVisibleWidth(part) <= 12)).toBe(true);
    });
});