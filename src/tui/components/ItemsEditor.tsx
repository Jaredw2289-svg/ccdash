...
export const ItemsEditor: React.FC<ItemsEditorProps> = ({
    ...
    const widthDetectionAvailable = statuslineWidthProbe?.reliable ?? canDetectTerminalWidth('statusline');
    ...
    return (
        <Box flexDirection='column'>
            ...
            {hasFlexSeparator && !widthDetectionAvailable && (
                <Box marginTop={1}>
                    <Text color='yellow'>⚠ Note: Live hook width is currently estimated from the fallback width.</Text>
                    <Text dimColor>{`  Flex separators and wrapping will follow the ${settings.fallbackTerminalWidth}-column fallback until hook width is reliable.`}</Text>
                </Box>
            )}
            {widgetPicker && (
                <Box marginTop={1} flexDirection='column'>
