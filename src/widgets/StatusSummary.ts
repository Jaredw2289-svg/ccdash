...
export class StatusSummaryWidget implements Widget {
    ...
    render(item: WidgetItem, context: RenderContext, settings: Settings): string | null {
        ...
        if (context.isPreview) {
            if (useGoalSummary) {
                return item.rawValue ? 'Refine the session dashboard information hierarchy' : 'Focus: Refine the session dashboard information hierarchy';
            }
            if (useCurrentFocus) {
                return item.rawValue ? 'Updating the renderer to wrap instead of hiding widgets' : 'Step: Updating the renderer to wrap instead of hiding widgets';
            }
            if (useLastConclusion) {
                return item.rawValue ? 'Wrapped narrow layouts without dropping dashboard context' : 'Step: Wrapped narrow layouts without dropping dashboard context';
            }
            if (useFirstHuman) {
                return item.rawValue ? 'Three-line layout, rich quota display' : 'Focus: Three-line layout, rich quota display';
            }
        ...
        if (dashboardState) {
            if (useGoalSummary) {
                ...
                if (goalSummary) {
                    return item.rawValue ? goalSummary : `Focus: ${goalSummary}`;
                }
            }
            ...
            if (useCurrentFocus || useLastConclusion) {
                ...
                if (now) {
                    return item.rawValue ? now : `Step: ${now}`;
                }
            }
        }
        ...
        if (transcriptPath) {
            ...
            if (transcriptSummary) {
                const label = useGoalLikeSummary ? 'Focus: ' : useNowLikeSummary ? 'Step: ' : 'Task: ';
