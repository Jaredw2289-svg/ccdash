...
function getContextBarWidth(displayMode: DisplayMode, responsiveTier?: RenderContext['responsiveTier']): number {
    const tier = responsiveTier ?? 'wide';
    if (displayMode === 'progress') {
        return tier === 'narrow' ? 16 : tier === 'medium' ? 24 : 32;
    }

    return tier === 'narrow' ? 8 : tier === 'medium' ? 12 : 16;
}
...
export class ContextBarWidget implements Widget {
    ...
    render(item: WidgetItem, context: RenderContext, settings: Settings): string | null {
        ...
        const barWidth = getContextBarWidth(displayMode, context.responsiveTier);
