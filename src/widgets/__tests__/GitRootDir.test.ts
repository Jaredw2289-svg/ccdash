...
function render(options: { cwd?: string; hideNoGit?: boolean; fallbackToCwd?: boolean; rawValue?: boolean; isPreview?: boolean } = {}) {
    const widget = new GitRootDirWidget();
    const context: RenderContext = {
        isPreview: options.isPreview,
        data: options.cwd ? {
            cwd: options.cwd,
            workspace: { project_dir: options.cwd }
        } : undefined
    };
    const item: WidgetItem = {
        id: 'git-root-dir',
        type: 'git-root-dir',
        rawValue: options.rawValue ?? true,
        metadata: {
            ...(options.hideNoGit ? { hideNoGit: 'true' } : {}),
            ...(options.fallbackToCwd ? { fallbackToCwd: 'true', hideWhenEmpty: 'true' } : {})
        }
    };
...
describe('GitRootDirWidget', () => {
    ...
    it('falls back to workspace project dir when configured', () => {
        mockExecSync.mockImplementation(() => { throw new Error('No git'); });

        expect(render({ cwd: '/tmp/worktree/my-repo', fallbackToCwd: true, rawValue: true })).toBe('my-repo');
    });
    ...
    it('should enable raw value support', () => {
        const widget = new GitRootDirWidget();
