...
describe('StatusSummaryWidget', () => {
    ...
    it('truncates mixed-width text by display width', () => {
        mockReadSessionStatusFile.mockReturnValue({
            goal: null,
            now: '发布完成后立刻更新 README and announce the npm release',
            updatedAt: '2026-03-10T00:00:00.000Z'
        });

        const itemWithMaxWidth: WidgetItem = { id: 'summary', type: 'status-summary', rawValue: true, maxWidth: 12 };
        const context: RenderContext = { data: { session_id: 'session-abc' } };
        const result = render(widget, itemWithMaxWidth, context);
    ...
    describe('first-human source', () => {
        ...
        it('shows goal-like preview text', () => {
            ...
            expect(render(widget, labeledItem, { isPreview: true })).toBe('Focus: Three-line layout, rich quota display');
        });
    });
    ...
    describe('dashboard state sources', () => {
        it('renders goal summary from dashboard state', () => {
            ...
            expect(render(widget, item, context)).toBe('Focus: Refine the session dashboard information hierarchy');
        });
        ...
        it('renders now from dashboard state via current-focus source', () => {
            ...
            expect(render(widget, item, context)).toBe('Step: Updating the renderer to wrap instead of hiding widgets');
        });
        ...
        it('renders now from dashboard state via last-conclusion source', () => {
            ...
            expect(render(widget, item, context)).toBe('Step: Wrapped narrow layouts without dropping dashboard context');
        });
    ...
    describe('transcript fallback', () => {
        ...
        it('reads goal from Claude-managed session status for goal source', () => {
            ...
            expect(render(widget, item, context)).toBe('Focus: Understand available capabilities in ccdash');
        });
    });
});