...
describe('ContextBarWidget', () => {
    ...
    it('compacts context bars across responsive tiers', () => {
        const context: RenderContext = {
            data: {
                context_window: {
                    context_window_size: 200000,
        ...
        expect(widget.render({ id: 'ctx', type: 'context-bar' }, {
            ...context,
            responsiveTier: 'medium'
        }, DEFAULT_SETTINGS)).toBe('Context: [bar:15.0:12] 30k/200k (15%)');
        expect(widget.render({ id: 'ctx', type: 'context-bar' }, {
            ...context,
            responsiveTier: 'narrow'
        }, DEFAULT_SETTINGS)).toBe('Context: [bar:15.0:8] 30k/200k (15%)');
        expect(widget.render({
            id: 'ctx',
            type: 'context-bar',
            metadata: { display: 'progress' }
            ...
            ...context,
            responsiveTier: 'medium'
        }, DEFAULT_SETTINGS)).toBe('Context: [bar:15.0:24] 30k/200k (15%)');
    });
