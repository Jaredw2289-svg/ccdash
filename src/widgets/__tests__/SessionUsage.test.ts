...
describe('SessionUsageWidget', () => {
    ...
    it('compacts reset suffixes across responsive tiers in preview mode', () => {
        ...
        expect(render(widget, item, { isPreview: true })).toBe('Session: [bar:20.0:16] 20.0% · 3h45m left');
        expect(render(widget, item, { isPreview: true, responsiveTier: 'medium' })).toBe('Session: [bar:20.0:12] 20.0% · 3h45m');
        expect(render(widget, item, { isPreview: true, responsiveTier: 'narrow' })).toBe('Session: [bar:20.0:8] 20.0%');
    });
});