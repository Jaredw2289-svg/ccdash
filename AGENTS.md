...
- Output style: one short English sentence per field, high-level, no chain-of-thought, no markdown list markers, no surrounding quotes.
...
- `goal`: update only when the stable session goal meaningfully changes. Do not rewrite it on every turn.
- Privacy: never write hidden reasoning or private scratch work.

Write it with:

```bash
bun run src/ccstatusline.ts --write-session-status --now "<high-level now summary>" --goal "<stable session goal>"
```
