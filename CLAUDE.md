...
After every assistant response in this repository, update the per-session dashboard status.

- Purpose: keep both `Focus:` and `Step:` accurate for the current Claude Code session.
- Focus is **fully Claude-managed** — if you don't write one, it stays empty. No automatic derivation from user prompts.
...
- `step`: always update with the latest visible conclusion, completed step, or current action.
- `focus`: **required on the first turn** of each session. Should be a distilled session objective, not a paraphrase of the user's question. Update only when the stable session focus meaningfully changes.
  - Good: `Remove goal fallback derivation from dashboard` / `为状态栏添加响应式布局`
  - Bad: `User asked to remove goal fallback` / `知道, 好像 goal 和 now...`
